...
local PARAM is readJson("param.json")["UTIL_TERM"].
...
local Ts is get_param(PARAM, "TS", 0.25).
local scan_timeout_max is get_param(PARAM, "SCAN_TIMEOUT", 10).

lock AG to AG1.
local prev_AG is AG.
local nAG is 0. // number of times action group was pressed
